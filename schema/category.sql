...
(
    ...
    `status`     VARCHAR(20) NOT NULL DEFAULT 'active' COMMENT 'Typed Role lifecycle status; allowed values are active or inactive',
    `created_at` DATETIME NOT NULL COMMENT 'UTC timestamp assigned by the Category application when the Role is created',
    `updated_at` DATETIME NOT NULL COMMENT 'UTC timestamp assigned by the Category application on Role mutation, soft delete, or restore',
    `deleted_at` DATETIME NULL COMMENT 'UTC timestamp assigned by the Category application on soft delete; NULL means not soft deleted',
    ...
    CONSTRAINT `chk_maa_category_category_image_roles_status` CHECK (`status` IN ('active', 'inactive'))
)
ENGINE = InnoDB
DEFAULT CHARSET = utf8mb4
COLLATE = utf8mb4_unicode_ci
...
(
    ...
    `updated_at`             DATETIME NOT NULL COMMENT 'UTC timestamp assigned by the Category application on field mutation, ordering, soft delete, or restore',
    `deleted_at`             DATETIME NULL COMMENT 'UTC timestamp assigned by the Category application on soft delete; NULL means not deleted',
