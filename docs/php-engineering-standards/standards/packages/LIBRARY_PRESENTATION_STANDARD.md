...
> Final Release Candidate MUST prepare the badge markup for publication before Tag creation when the package is intended to be published immediately after owner approval. However, live Packagist badges MUST NOT be exposed before the package is actually published and the version name is valid.
...
Before the first Stable Tag, you MUST NOT label a version badge as `Latest Version` if no stable release exists. The default Packagist version badge cannot be relied upon to display pre-releases without configuration. Using the `include_prereleases` parameter is optional and only permitted when there is an explicit decision to display a Pre-release version clearly labeled as such, not as Stable.
A Final Release Candidate MAY prepare the badge markup required for publication, but it MUST NOT expose live Packagist badges with inaccurate claims before the conditions are met.
